...
exports.updateSettings = exports.getSettings = exports.getAnalytics = exports.getDashboardStats = exports.updateTripStatus = exports.addRoute = exports.getRoutes = exports.getTripById = exports.getTrips = exports.createTripSchedule = exports.scheduleTrip = exports.getDrivers = exports.deleteDriver = exports.addDriver = exports.checkDuplicateDriver = exports.markAllNotificationsRead = exports.markNotificationRead = exports.getNotifications = exports.getMaintenanceHistory = exports.logMaintenance = exports.updateBusStatus = exports.getBuses = exports.addBus = void 0;
...
const addBus = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.invalidateTransporter(transporterId);
...
const getBuses = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.set(cacheKey, buses, 15 * 1000); // 15 seconds
...
const updateBusStatus = async (req, res) => {
    try {
        ...
        batch.update(firebase_1.db.collection('buses').doc(id), {
            status,
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
        // If driver needs to be unassigned
        if (driverId) {
            batch.update(firebase_1.db.collection('drivers').doc(driverId), {
                busAssignedId: null,
                busNumber: null,
                vehicleAssigned: '',
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        ...
        await batch.commit();
        cacheService_1.cacheService.invalidateTransporter(bus.transporterId);
...
const logMaintenance = async (req, res) => {
    try {
        ...
        await firebase_1.db.collection('maintenance').add({
            busId: id,
            busNumber,
...
exports.logMaintenance = logMaintenance;
const getMaintenanceHistory = async (req, res) => {
    try {
        const { id } = req.params;
        const snapshot = await firebase_1.db.collection('maintenance')
            .where('busId', '==', id)
            .orderBy('date', 'desc')
            .limit(5)
            .get();
...
const getNotifications = async (req, res) => {
    try {
        ...
        if (!transporterId)
            return res.status(400).json({ error: 'Transporter ID is required' });
        const snapshot = await firebase_1.db.collection('transporter_notifications')
...
const markNotificationRead = async (req, res) => {
    try {
        const id = req.params.id;
        await firebase_1.db.collection('transporter_notifications').doc(id).update({ read: true });
...
const checkDuplicateDriver = async (req, res) => {
    try {
...
exports.checkDuplicateDriver = checkDuplicateDriver;
const addDriver = async (req, res) => {
    let createdAuthUserId = null;
    try {
        const { email, password, name, phone, transporterId, status, cnic, licenseNumber, licenseType, licenseExpiry, address, emergencyContact, joiningDate, salary, employmentType, experienceYears } = req.body;
        ...
        // 2. Save minimal driver info to Firestore for legacy dependencies
        const batch = firebase_1.db.batch();
        const driverRef = firebase_1.db.collection('drivers').doc(driver.id);
        batch.set(driverRef, {
            uid: driver.id,
        ...
        const userRef = firebase_1.db.collection('users').doc(driver.id);
        batch.set(userRef, {
            uid: driver.id,
            fullName: trimmedName,
            email: normalizedEmail,
...
const deleteDriver = async (req, res) => {
    try {
        ...
        const batch = firebase_1.db.batch();
        batch.delete(firebase_1.db.collection('drivers').doc(id));
        batch.delete(firebase_1.db.collection('users').doc(id));
        await batch.commit();
        await supabase_1.supabaseAdmin.auth.admin.deleteUser(id).catch((error) => {
            console.warn('Failed to delete driver auth user:', error?.message || error);
        });
        if (driver.transporterId) {
            cacheService_1.cacheService.invalidateTransporter(driver.transporterId);
        }
...
const getDrivers = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.set(cacheKey, drivers, 15 * 1000); // 15 seconds
...
const generateTripSeats = (batch, tripId, totalSeats, fare) => {
    const seatsRef = firebase_1.db.collection('trips').doc(tripId).collection('seats');
...
const scheduleTrip = async (req, res) => {
    try {
        const { routeId, driverId, busId, departureTime, arrivalTime, price, totalSeats, transporterId, status, startDate, endDate, selectedDays } = req.body;
        // Check if it's an array of trips
        if (Array.isArray(req.body.trips)) {
            const tripsData = req.body.trips;
            const createdTrips = [];
            const batch = firebase_1.db.batch(); // Firestore batch
            // Process each trip
            for (const t of tripsData) {
                ...
                const fsTripRef = firebase_1.db.collection('trips').doc(pgTrip.id);
                batch.set(fsTripRef, {
                    ...t,
                    id: pgTrip.id,
                    createdAt: admin.firestore.FieldValue.serverTimestamp(),
...
const createTripSchedule = async (req, res) => {
    try {
        const data = req.body;
        ...
        const ref = await firebase_1.db.collection('tripSchedules').add(scheduleData);
...
exports.createTripSchedule = createTripSchedule;
const getTrips = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.set(cacheKey, mappedTrips, 10 * 1000); // 10 seconds
...
const getRoutes = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.set(cacheKey, routes, 60 * 1000); // 60 seconds
...
const addRoute = async (req, res) => {
    try {
        ...
        const ref = await firebase_1.db.collection('routes').add(routeData);
        cacheService_1.cacheService.invalidateTransporter(transporterId);
...
const updateTripStatus = async (req, res) => {
    try {
        ...
        const tripRef = firebase_1.db.collection('trips').doc(id);
        batch.update(tripRef, { status, updatedAt: now });
        if (busId) {
            let busStatus = 'available';
            if (status === 'in_progress' || status === 'active')
                busStatus = 'on_trip';
            batch.update(firebase_1.db.collection('buses').doc(busId), {
                status: busStatus,
        ...
        // Create Activity Log
        const activityRef = firebase_1.db.collection('trip_activities').doc();
        ...
        if (busId) {
            ...
            if (trip && trip.bus) {
                cacheService_1.cacheService.invalidateTransporter(trip.bus.transporterId);
            }
        }
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        cacheService_1.cacheService.set(cacheKey, responseData, 10 * 1000); // 10 seconds
...
const getAnalytics = async (req, res) => {
    try {
        ...
        const ratingsSnapshot = await firebase_1.db.collection('ratings').where('transporterId', '==', transporterId).get();
        const ratings = ratingsSnapshot.docs.map(doc => doc.data());
        // Fetch searches from Firestore (Legacy)
        const searchesSnapshot = await firebase_1.db.collection('searches').get();
...
const getSettings = async (req, res) => {
    try {
        ...
        const doc = await firebase_1.db.collection('settings').doc(String(transporterId)).get();
        let responseData;
        if (doc.exists) {
        ...
        cacheService_1.cacheService.set(cacheKey, responseData, 60 * 1000); // 60 seconds
        res.json(responseData);
    }
    catch (error) {
...
const updateSettings = async (req, res) => {
    try {
        ...
        if (!transporterId)
            return res.status(400).json({ error: 'Transporter ID is required' });
        await firebase_1.db.collection('settings').doc(String(transporterId)).set(req.body.settings, { merge: true });
        cacheService_1.cacheService.invalidateTransporter(transporterId);
        res.json({ success: true });
    }
    catch (error) {
